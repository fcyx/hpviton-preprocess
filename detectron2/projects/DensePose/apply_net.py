...
class ShowAction(InferenceAction):
    ...
    def execute_on_outputs(
        ...
        #image = entry["image"]
        cv2.imwrite('./image-densepose/1_1.png', image)
        image = np.tile(image[:, :, np.newaxis], [1, 1, 3])
        print("test:",image.max(),image[181,181],image.shape)
        data = extractor(outputs)
        #print("data:",data) 
        image_vis = visualizer.visualize(image, data)
        print("test1:",image_vis.max(),image_vis[181,181],image_vis.shape)
        entry_idx = context["entry_idx"] + 1
        #out_fname = cls._get_out_fname(entry_idx, context["out_fname"])
        #out_dir = os.path.dirname(out_fname)
        #from matplotlib import pyplot as plt
        #plt.hist(image_vis.ravel(),256,[0,256],);
        hist = cv2.calcHist([image_vis],[0],None,[25],[0,255])
        print((hist>0.0).sum())
        out_fname = './image-densepose/' + image_fpath.split('/')[-1]
        out_dir = './image-densepose'
        if len(out_dir) > 0 and not os.path.exists(out_dir):
            os.makedirs(out_dir)
        cv2.imwrite('./image-densepose/test.png', image)
