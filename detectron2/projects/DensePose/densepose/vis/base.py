...
class MatrixVisualizer:
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        matrix1 = np.tile(matrix[:, :, np.newaxis], [1, 1, 3])
        matrix_scaled = matrix.astype(np.float32) * self.val_scale
        _EPSILON = 1e-6
        print("visvis:",mask,mask.shape,mask.max(),mask_bg,mask_bg.shape,mask_bg.max(),matrix,matrix.shape,matrix.max())
        ...
        #matrix1[mask_bg] = image_target_bgr[y : y + h, x : x + w, :][mask_bg]
