...
def get_im_parse_agnostic(im_parse, pose_data, w=768, h=1024):
    ...
    print("im_parse",label_array)
    print("label_array1111111:",np.unique(np.array(label_array)))
    ...
    for parse_id, pose_ids in [(14, [2, 5, 6, 7]), (15, [5, 2, 3, 4])]:
        ...
        for i in pose_ids[1:]:
            ...
            i_prev = i
        print("label_array:",np.unique(np.array(label_array)))
        print(" parse_id:", parse_id)
        print("np.array(mask_arm)",np.array(mask_arm))
...
if __name__ =="__main__":
    ...
    for im_name in tqdm(os.listdir(osp.join(data_path, 'image'))):
        ...
        #im_parse = cv2.imread(osp.join(data_path, 'image-parse-v3', parse_name))
        #print("111111111111111",im_parse1.max())
        img_cv1  = cv2.imread(osp.join(data_path, 'image', im_name))
        shapee = img_cv1.shape
        w = shapee[0]
        h = shapee[1]
        print("w",w)
        print("h",h)
        print(np.array(im_parse).max())
        agnostic = get_im_parse_agnostic(im_parse, pose_data,h,w)
